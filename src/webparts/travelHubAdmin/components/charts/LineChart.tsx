...
export const LineChart: React.FC<ILineChartProps> = ({ series, width = 480, height = 220 }) => {
  ...
  const gridLines = [0, 1, 2, 3, 4].map((g) => {
    ...
    return <line key={g} x1={padding.left} y1={gy} x2={width - padding.right} y2={gy} stroke={FIXED_PALETTE.border} strokeWidth={1} />;
  });
  ...
  const xLabels = series.map((d, i) => {
    ...
    return <text key={i} x={x} y={height - 6} fontSize={10} fill={FIXED_PALETTE.textMuted} textAnchor="middle">{label}</text>;
  });
  ...
  return (
    <div>
      <svg viewBox={`0 0 ${width} ${height}`} width="100%" height={height} preserveAspectRatio="none">
        {gridLines}
        <polyline points={pointsFor('pageViews')} fill="none" stroke={FIXED_PALETTE.secondary} strokeWidth={2.5} />
        <polyline points={pointsFor('uniqueUsers')} fill="none" stroke={FIXED_PALETTE.primary} strokeWidth={2.5} />
        {xLabels}
      </svg>
      <div className={styles.legend}>
        <span><i style={{ background: FIXED_PALETTE.secondary }} /> Page Views</span>
        <span><i style={{ background: FIXED_PALETTE.primary }} /> Unique Users</span>
      </div>
    </div>
  );
};
